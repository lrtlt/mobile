...
The [React Native Upgrade Helper](https://react-native-community.github.io/upgrade-helper/) uses pre-computed diffs from [react-native-community/rn-diff-purge](https://github.com/react-native-community/rn-diff-purge).
...
- When applying diffs, replace these template names with the actual project's app name and package identifiers.
- Binary files (e.g. Gradle wrapper JAR) appear as binary patch deltas and should be downloaded directly from the target version's branch instead of applying the diff.
